...
public class SmsHelper {
     ...
     * @param body    Body of incoming SMS message
     */
    public static Uri addMessageToInboxAsRead(Context context, String address, String body, boolean asRead) {
        ...
        cv.put("seen", true);
        cv.put("error_code", BT_ERROR_CODE);
        if(asRead) {
            cv.put("read", true);
        }
    ...
    public static List<Message> deleteBluetoothMessages(Context context, boolean afterTime) {
        ...
        if(afterTime) { selection = BT_TEMP_SELECTION + " AND date_sent < " + (System.currentTimeMillis()-21600000); } else { selection = BT_TEMP_SELECTION; }
